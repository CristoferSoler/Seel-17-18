...
def componentGroupDetail(request, id):
    # connect to db and get ComponentGroup data
    all_componentGroups = ComponentGroup.objects.all()
    context = {'all_componentGroups': all_componentGroups}
    # http response is implemented inside render
    return render(request, 'model/ComponentGroup.html', context)
