...
The cleanup activities begin with the affected network areas being isolated. The goal is to completely deprive the attacker of access to the IT environment. This should be done in one fell swoop so that the attacker does not notice prematurely that he has been discovered and may take countermeasures. Therefore, the network sections must be isolated in a concerted action, the so-called cut-off.
