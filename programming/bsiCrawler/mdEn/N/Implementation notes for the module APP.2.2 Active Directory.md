...
Because service administrators need extensive permissions for service management, they should be able to perform administrative data management activities. Conversely, the data administrators should not be able to change the configuration of the Active Directory.
...
* Allow computer and user accounts to be trusted for delegation: Administrators
...
Fine-grained password policies have been introduced with Windows Server 2008, allowing different password policy and blocking policies to be defined for different user groups within a domain.
...
Domains that are located in several trees, that is, span different namespaces, can still be managed together.
...
In addition, in the event of an emergency, a domain administrator account (eg the default DA with a strong password) should be set up and stored securely and at the same time easily accessible in case none of the DAs are available.
...
The number of systems that ADS administrators log on to should be limited as much as possible. When AD administrators log on only to the systems they actually manage, and indirectly to select few administration workstations, the places where valuable credentials are tapped can be severely curtailed. Therefore, server administrator accounts should not be used on workstations, domain administrator accounts should not be used on workstations or servers. It should be excluded as technically possible that a privileged account is used to log in to a system of another layer.
...
In each domain of the forest, the default administrator account is created during the installation. As a standard account, this user account is particularly vulnerable to attacks. Since the administrator account can not be disabled or deleted, it should be renamed as a safeguard. When renaming, make sure that the description of the administrator account is also changed. After the account has been renamed, you should then set up an unprivileged account named "Administrator" that should not be used in daily operations. When evaluating the log files, it is possible to detect whether there were successful or unsuccessful logins to this unprivileged user account. This would indicate an attack attempt.
...
Until Windows Server 2008 had accidentally deleted AD objects or those that were intentionally deleted, but still needed later, consuming and error-prone restore from backups. In Windows Server 2008 R2, a Trash Bin was introduced, but it could only be operated from the command line. Only since Windows Server 2012 is there an easy-to-use recycle bin, which can be operated both from the command line via PowerShell and via GUI.
...
With such monitoring, not only can the sprawling storage space for the Active Directory database be tracked, but also object overflow attacks can be detected in which the storage footprint increases dramatically in a relatively short time.
...
In addition, the members of the Backup Operators group should be limited to users required to back up the system files. Users responsible for backing up application data should not be a member of the domain controller's Backup Operators group. Rather, these users should be registered as members in the local group "Backup Operators" of the respective application server.
