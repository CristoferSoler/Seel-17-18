...
Generell wiki behaviour as found on i.e. wikipedia.org applies:
\begin{itemize}
    \item each word on any page can only link to one other page at a time,
    \item users who collaboratively work on the same wiki page can edit or delete each others links
        .
\end{itemize}
\end{tcolorbox}

\begin{comment}
What is different from wikipedia is the way links are shown on BSI catalogue pages.
While normal articles and news have normal word links - that means a word has a different colour, usually blue, to indicate that it is a link - BSI pages should have markers on the side which contain the links.
