...
If checksums are offered for a software package, these should be checked before the package is installed.
...
* These specifications come from overarching planning and are determined by the general objectives. The more precisely the framework conditions are known and the more precisely the specifications are formulated, the easier the following planning steps become.
...
Overvoltages can enter into a building and the IT operated there, not just via the cables laid in the building, but also via all electrically conductive external connections such as telephone, water or gas pipes. In addition, overvoltages can also be coupled to internal lines.
...
In its Part 2 "Risk Management", DIN EN 62305 describes in general terms the way to a risk-oriented lightning and surge protection. Part 3 deals with the "Protection of Structures and Persons", in Part 4 "Electrical and Electronic Systems in Buildings".
...
When carrying out the security check, the administrators should document their steps in such a way that they can be understood (for example, in the event of a suspected compromised system). The results of the safety check must be documented, deviations from the "target state" must be investigated.
...
 If a new version of the server software is to be used on the new system, it must be ensured that the new version can handle the existing data correctly. This not only concerns the task of correctly importing data from the old version, but also, in particular, modifying this data or adding new data records. Especially in such cases often problems arise, so that thorough testing recommended werden.Außerdem it is important that the configuration of the old service on the new system can be correctly taken over or at least "functionally equivalent reconstructed".
...
One of the requirements for using load balancing is that the respective applications or services must not require write access to data.
