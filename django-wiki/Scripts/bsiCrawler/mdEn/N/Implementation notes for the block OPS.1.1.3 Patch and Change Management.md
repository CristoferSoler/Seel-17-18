...
There are different products that support the patch and change management process. In order to make an appropriate choice from these products, the requirements for these tools, such as which platforms to support, must be specified prior to procurement (see OPS.1.1.3.M8 * Safe Use of Change Management Tools *).
...
 Many manufacturers offer to receive the necessary information about new hardware or software or about errors that have occurred and their rectification by subscription by e-mail.
...
 In-house software development could be another way to make software changes if any security vulnerabilities or other requirements make it necessary. However, not only the necessary expertise must be available. The interfaces or the source code must also be open.
* Testing
...
Not every amendment is treated as a normal change within the change process. Some routine changes, which are clearly circumscribed, standardized, and yet modest, can be treated as a service request. For example, a service request would be to reset a password and, in terms of change management, to change a login banner of a service (the text the service uses to log in through the network interface).
...
 This category includes changes that require significant effort and have a significant impact on IT services. Such changes are discussed in the CAB to define the required effort and to minimize the risk. In the run-up to the meeting, the necessary documentation is first sent to the members of the CAB and, if necessary, to a number of IT specialists and developers.
...
#### OPS.1.1.3.M9 Testing and Acceptance Procedure for New Hardware and Software [IT Leader]
...
In the case of software acceptance, a distinction is made between software that was developed by itself or on assignment, and standard software that is adapted only for the specific application.
...
Furthermore, not all checksum comparisons can be performed without the involvement of the user, since the checksums, signatures or certificates required for this purpose are not provided by the manufacturers in a uniform manner. This often requires manual verification on the manufacturer pages or customization of the URLs in the change software.
