...
As part of the WLAN design, which follows the documented strategic requirements, potentially disruptive systems near the future location of access points must be identified and evaluated. If microwaves are operated near access points or other IT systems, interference may occur. Other possible sources of interference can Bluetooth transmitter, power lines, cordless telephones (DECT) or LCD monitors as well as its building material itself.
...
Even for access points that are administered without a controller, only authorized access points of the institution that have been incorporated into the established processes may be connected to the network. Use IEEE 802.1X to ensure that this requirement is met. The legitimate access of terminals to the WLAN should also be checked using IEEE 802.1X and EAP-TLS.
...
The IT operation should be informed or trained about the hazards to which WLAN components are exposed and the security measures to be observed.
