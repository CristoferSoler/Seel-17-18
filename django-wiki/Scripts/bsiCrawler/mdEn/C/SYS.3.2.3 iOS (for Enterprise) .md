...
Based on the purpose of the application and the need for protection, the time interval for the device's "automatic lock" MUST be set to the lowest possible value. A low value ensures that no unauthorized use of the unattended device is possible. A reasonably short auto-lock period assists the user in complying with the institution's security policies, provided the device does not linger in an unlocked state through interaction with the user interface. When defining the time period until the passcode query, the requirements for protection requirements and usability MUST be observed.
...
In order to prevent an unauthorized deletion of configuration profiles, suitable regulations MUST be made and implemented by those responsible. For example, the deletion can be technically realized by password-protected authentication or organizationally prohibited.
